...
async def status(db: AsyncSession = Depends(get_db)):
    ...
    onboarded_at = (await db.execute(text("SELECT onboarded_at FROM org_profile WHERE id = 1"))).scalar()
    advanced = (await db.execute(text(
        "SELECT COUNT(*) FROM controls WHERE status <> 'not_started'"))).scalar() or 0
    evidence = (await db.execute(text("SELECT COUNT(*) FROM evidence_items"))).scalar() or 0
    gaps = (await db.execute(text("SELECT COUNT(*) FROM gap_findings"))).scalar() or 0
    members = (await db.execute(text("SELECT COUNT(*) FROM workspace_members"))).scalar() or 0
...
async def commit(
    ...
    return await onboarding.commit(db, body.profile.model_dump(), [i.model_dump() for i in body.items], x_actor)
