...
async def update_control(
    ...
    current = (await db.execute(text(
        "SELECT status, owner, description FROM controls WHERE requirement_id = :rid"
    ...
    fields, params = [], {"rid": requirement_id}
    ...
    await db.execute(text(
        f"UPDATE controls SET {', '.join(fields)} WHERE requirement_id = :rid"
    ), params)
    ...
    await db.execute(text("""
        ...
        VALUES ('control', :rid, :action, :actor, :summary, CAST(:detail AS jsonb))
    """), {
        "rid": requirement_id, "action": action, "actor": x_actor or "unknown",
        "summary": summary,
        "detail": json.dumps({k: str(v) for k, v in params.items() if k not in ("rid", "actor")}),
    })
    await db.commit()
    return {"ok": True}
...
async def ai_draft(
    ...
    try:
        return await draft_control(db, requirement_id, provider)
